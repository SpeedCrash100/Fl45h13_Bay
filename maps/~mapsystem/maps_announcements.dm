...
/datum/map/proc/emergency_shuttle_called_announcement()
	GLOB.evacuation_controller.evac_called.Announce(replacetext(emergency_shuttle_called_message, "%ETA%", "[round(GLOB.evacuation_controller.get_eta()/60)] minute\s."), new_sound = emergency_shuttle_called_sound)

/datum/map/proc/grid_check_announcement()
	GLOB.command_announcement.Announce(replacetext(grid_check_message, "%STATION_NAME%", station_name()), "Automated Grid Check", new_sound = grid_check_sound)

/datum/map/proc/grid_restored_announcement()
	GLOB.command_announcement.Announce(replacetext(grid_restored_message, "%STATION_NAME%", station_name()), "Power Systems Nominal", new_sound = grid_restored_sound)
...
/datum/map/proc/level_x_biohazard_announcement(var/bio_level)
	...
	GLOB.command_announcement.Announce("Confirmed outbreak of level [bio_level] biohazard aboard the [station_name()]. All personnel must contain the outbreak.", "Biohazard Alert", new_sound = level_x_biohazard_sound(bio_level))

/datum/map/proc/level_x_biohazard_sound(var/bio_level)
	return

/datum/map/proc/meteors_detected_announcement()
	GLOB.command_announcement.Announce(replacetext(meteor_detected_message, "%STATION_NAME%", station_name()), "[station_name()] Sensor Array", new_sound = meteor_detected_sound)

/datum/map/proc/radiation_detected_announcement()
	GLOB.command_announcement.Announce(replacetext(radiation_detected_message, "%STATION_NAME%", station_name()), "Anomaly Alert", new_sound = radiation_detected_sound)

/datum/map/proc/space_time_anomaly_detected_annoncement()
	GLOB.command_announcement.Announce("Space-time anomalies have been detected on the [station_name()].", "Anomaly Alert", new_sound = space_time_anomaly_sound)

/datum/map/proc/unidentified_lifesigns_announcement()
	GLOB.command_announcement.Announce(replacetext(unidentified_lifesigns_message, "%STATION_NAME%", station_name()), "Lifesign Alert", new_sound = unidentified_lifesigns_sound)

/datum/map/proc/unknown_biological_entities_announcement()
	GLOB.command_announcement.Announce(replacetext(unknown_biological_entities_message, "%STATION_NAME%", station_name()), "Lifesign Alert", new_sound = command_report_sound)
