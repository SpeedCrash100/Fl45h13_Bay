...
/datum/event/prison_break/announce()
	if(areas && areas.len > 0)
		GLOB.command_announcement.Announce("[pick("Gr3y.T1d3 virus","Malignant trojan")] detected in [station_name()] [(eventDept == "Security")? "imprisonment":"containment"] subroutines. Secure any compromised areas immediately. [station_name()] AI involvement is recommended.", "[eventDept] Alert")
...
/datum/event/prison_break/start()
	...
	if(areas && areas.len > 0)
		...
		log_error("Could not initate grey-tide. Unable to find suitable containment area.")
		kill()
