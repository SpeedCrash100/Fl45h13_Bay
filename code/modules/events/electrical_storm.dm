...
/datum/event/electrical_storm/announce()
	...
	switch(severity)
		if(EVENT_LEVEL_MUNDANE)
			GLOB.command_announcement.Announce("A minor electrical storm has been detected near the [station_name()]. Please watch out for possible electrical discharges.", "[station_name()] Sensor Array")
		if(EVENT_LEVEL_MODERATE)
			GLOB.command_announcement.Announce("The [station_name()] is about to pass through an electrical storm. Please secure sensitive electrical equipment until the storm passes.", "[station_name()] Sensor Array", new_sound = GLOB.using_map.electrical_storm_moderate_sound)
		if(EVENT_LEVEL_MAJOR)
			GLOB.command_announcement.Announce("Alert. A strong electrical storm has been detected in proximity of the [station_name()]. It is recommended to immediately secure sensitive electrical equipment until the storm passes.", "[station_name()] Sensor Array", new_sound = GLOB.using_map.electrical_storm_major_sound)
...
/datum/event/electrical_storm/end()
	..()
	GLOB.command_announcement.Announce("The [station_name()] has cleared the electrical storm. Please repair any electrical overloads.", "Electrical Storm Alert")
