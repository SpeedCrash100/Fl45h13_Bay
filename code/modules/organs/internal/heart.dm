...
/obj/item/organ/internal/heart/proc/handle_pulse()
	if(owner.stat == DEAD || robotic >= ORGAN_ROBOT)
		...
		return
	if(owner.shock_stage >= 120 || owner.getOxyLoss() >= 100 || owner.get_effective_blood_volume() < GLOB.BLOOD_VOLUME_SURVIVE || prob(max(0, owner.getBrainLoss() - owner.maxHealth * 0.75))) // The heart has stopped due to going into traumatic or cardiovascular shock.
		if(pulse != PULSE_NONE)
		...
		if(owner.shock_stage > 30)
			pulse_mod++
		if(owner.get_effective_blood_volume() <= GLOB.BLOOD_VOLUME_BAD)	//how much blood do we have
