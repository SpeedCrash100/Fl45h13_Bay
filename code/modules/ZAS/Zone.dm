...
/zone/proc/add(turf/simulated/T)
	...
	ASSERT(!invalid)
	ASSERT(istype(T))
	ASSERT(!GLOB.air_master.has_valid_zone(T))
#endif
	...
	if(T.fire)
		...
		fire_tiles.Add(T)
		GLOB.air_master.active_fire_zones |= src
...
/zone/proc/tick()
	if(air.temperature >= PHORON_FLASHPOINT && !(src in GLOB.air_master.active_fire_zones) && air.check_combustability() && contents.len)
