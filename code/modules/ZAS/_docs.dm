...
Control Flow:
Every air tick:
	Marked GLOB.turfs are updated with update_air_properties(), followed by post_update_air_properties().
...
#define ZONE_MIN_SIZE 14 //zones with less than this many GLOB.turfs will always merge, even if the connection is not direct
