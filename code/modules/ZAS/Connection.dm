...
/connection/New(turf/simulated/A, turf/simulated/B)
	#ifdef ZASDBG
	ASSERT(GLOB.air_master.has_valid_zone(A))
	//ASSERT(GLOB.air_master.has_valid_zone(B))
	#endif
	src.A = A
	src.B = B
	zoneA = A.zone
	if(!istype(B))
		mark_space()
		edge = GLOB.air_master.get_edge(A.zone,B)
		edge.add_connection(src)
	else
		zoneB = B.zone
		edge = GLOB.air_master.get_edge(A.zone,B.zone)
		edge.add_connection(src)
...
//		log_debug("Invalid A.")
	...
	var/block_status = GLOB.air_master.air_blocked(A,B)
