...
Overview:
	...
	They are found in a zone's edges list and in GLOB.air_master.edges.
...
/connection_edge/proc/erase()
	GLOB.air_master.remove_edge(src)
...
/connection_edge/zone/tick()
	...
	if(equiv)
		if(direct)
			erase()
			GLOB.air_master.merge(A, B)
			return
		else
			A.air.equalize(B.air)
			GLOB.air_master.mark_edge_sleeping(src)

	GLOB.air_master.mark_zone_update(A)
	GLOB.air_master.mark_zone_update(B)
...
/connection_edge/zone/recheck()
	...
	if(!A.air.compare(B.air, vacuum_exception = 1))
		GLOB.air_master.mark_edge_active(src)
