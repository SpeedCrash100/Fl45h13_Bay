...
/turf/proc/update_air_properties()
	...
	for(var/d = 1, d < 16, d *= 2)
		...
		if(istype(unsim, /turf/simulated))
			...
			if(GLOB.air_master.has_valid_zone(sim))

				GLOB.air_master.connect(sim, src)
...
/turf/simulated/update_air_properties()
		...
		if(istype(unsim, /turf/simulated))
			...
			if(GLOB.air_master.has_valid_zone(sim))
				...
				if(!zone)
					...
					GLOB.air_master.connect(src, sim)
	...
	if(!GLOB.air_master.has_valid_zone(src)) //Still no zone, make a new one.
...
/turf/simulated/return_air()
	if(zone)
		if(!zone.invalid)
			GLOB.air_master.mark_zone_update(zone)
			return zone.air
		else
			if(!air)
				make_air()
