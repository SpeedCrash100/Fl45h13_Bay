...
/datum/seed/proc/randomize()
	...
	set_trait(TRAIT_PRODUCT_ICON,pick(GLOB.plant_controller.plant_product_sprites))
	set_trait(TRAIT_PLANT_ICON,pick(GLOB.plant_controller.plant_sprites))
...
/datum/seed/proc/harvest(var/mob/user,var/yield_mod,var/harvest_sample,var/force_amount)
	...
	if(!force_amount && get_trait(TRAIT_YIELD) == 0 && !harvest_sample)
		...
		if(name == "new line" || !(name in GLOB.plant_controller.seeds))
			uid = GLOB.plant_controller.seeds.len + 1
			name = "[uid]"
			GLOB.plant_controller.seeds[name] = src
...
/datum/seed/proc/update_growth_stages()
	if(get_trait(TRAIT_PLANT_ICON))
		growth_stages = GLOB.plant_controller.plant_sprites[get_trait(TRAIT_PLANT_ICON)]
	else
		growth_stages = 0
