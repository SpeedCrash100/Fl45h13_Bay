...
/obj/item/weapon/reagent_containers/food/snacks/grown/Initialize(mapload, planttype)
	...
	seed = GLOB.plant_controller.seeds[plantname]
...
/obj/item/weapon/reagent_containers/food/snacks/grown/proc/update_desc()
	...
	if(GLOB.plant_controller.product_descs["[seed.uid]"])
		desc = GLOB.plant_controller.product_descs["[seed.uid]"]
	else
		...
		if(seed.seed_noun == "spores")
			...
			desc += " fruit"
		GLOB.plant_controller.product_descs["[seed.uid]"] = desc
	desc += ". Delicious! Probably."

/obj/item/weapon/reagent_containers/food/snacks/grown/update_icon()
	if(!seed || !GLOB.plant_controller || !GLOB.plant_controller.plant_icon_cache)
		return
	overlays.Cut()
	...
	if(GLOB.plant_controller.plant_icon_cache[icon_key])
		plant_icon = GLOB.plant_controller.plant_icon_cache[icon_key]
	else
		...
		GLOB.plant_controller.plant_icon_cache[icon_key] = plant_icon
	overlays |= plant_icon
...
/obj/item/weapon/reagent_containers/food/snacks/fruit_slice/New(var/newloc, var/datum/seed/S)
	...
	overlays |= GLOB.fruit_icon_cache["rind-[rind_colour]"]
	if(!GLOB.fruit_icon_cache["slice-[rind_colour]"])
		var/image/I = image(icon,"fruit_slice")
		I.color = flesh_colour
		GLOB.fruit_icon_cache["slice-[rind_colour]"] = I
	overlays |= GLOB.fruit_icon_cache["slice-[rind_colour]"]
