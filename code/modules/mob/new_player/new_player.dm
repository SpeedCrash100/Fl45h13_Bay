	...
	if(!IsGuestKey(src.key))
		...
		if(GLOB.dbcon.IsConnected())
			...
			if(src.client && src.client.holder)
				isadmin = 1
			var/DBQuery/query = GLOB.dbcon.NewQuery("SELECT id FROM erro_poll_question WHERE [(isadmin ? "" : "adminonly = false AND")] Now() BETWEEN starttime AND endtime AND id NOT IN (SELECT pollid FROM erro_poll_vote WHERE ckey = \"[ckey]\") AND id NOT IN (SELECT pollid FROM erro_poll_textreply WHERE ckey = \"[ckey]\")")
			query.Execute()
			var/newpoll = 0
			while(query.NextRow())
				newpoll = 1
...
/mob/new_player/proc/LateChoices()
	...
	dat += "</center>"
	show_browser(src, jointext(dat, null), "window=latechoices;size=300x640;can_close=1")
