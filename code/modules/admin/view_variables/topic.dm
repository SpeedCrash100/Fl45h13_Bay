...
//Stuff below aren't in dropdowns/etc.
	...
	//~CARN: for renaming mobs (updates their name, real_name, mind.name, their ID/PDA and datacore records).
		...
		if(href_list["rename"])
			...
			if(alert(usr, "Your selected name contains words restricted by IC chat filters. Confirm this new name?", "IC Chat Filter Conflict", "Confirm", "Cancel") == "Cancel")
				return
			...
			var/mob/living/carbon/human/monkey/M = locate(href_list["makehuman"]) in GLOB.mob_list
			if(!istype(M))
