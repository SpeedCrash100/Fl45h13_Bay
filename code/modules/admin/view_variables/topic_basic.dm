...
/client/proc/vv_do_basic(datum/target, href_list)
	...
	if(check_rights(R_ADMIN, FALSE))
		if(href_list[VV_HK_EXPOSE])
			...
			to_chat(C, "[holder.stealthy_ ? "an Administrator" : "[usr.client.key]"] has granted you access to view a View Variables window")
	...
	if(href_list[VV_HK_MARK])
		usr.client.holder.marked_datum_weak = weakref(target)
	if(href_list[VV_HK_CALLPROC])
		usr.client.callproc_targetpicked(TRUE, target)
