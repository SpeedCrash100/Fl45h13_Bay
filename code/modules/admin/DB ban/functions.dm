datum/admins/proc/DB_staffwarn_record(var/ckey, var/reason)
	...
	var/DBQuery/query = GLOB.dbcon.NewQuery("SELECT id FROM erro_player WHERE ckey = '[dbckey]'")
	query.Execute()
	var/playerid = -1
	if(query.NextRow())
	...
	if(playerid == -1)
		...
		return
	query = GLOB.dbcon.NewQuery("UPDATE erro_player SET staffwarn='[dbreason]' WHERE id=[playerid]")
	query.Execute()
...
datum/admins/proc/DB_staffwarn_remove(var/ckey)
	...
	var/DBQuery/query = GLOB.dbcon.NewQuery("UPDATE erro_player SET staffwarn=NULL WHERE ckey='[dbckey]'")
	query.Execute()
...
/proc/_DB_ban_record(var/a_ckey, var/a_computerid, var/a_ip, var/bantype, var/mob/banned_mob, var/duration = -1, var/reason, var/job = "", var/rounds = 0, var/banckey = null, var/banip = null, var/bancid = null)
	...
	var/DBQuery/query = GLOB.dbcon.NewQuery("SELECT id FROM erro_player WHERE ckey = '[ckey]'")
	query.Execute()
	var/validckey = 0
	if(query.NextRow())
		validckey = 1
...
datum/admins/proc/DB_ban_edit(var/banid = null, var/param = null)
	...
	var/DBQuery/query = GLOB.dbcon.NewQuery("SELECT ckey, duration, reason FROM erro_ban WHERE id = [banid]")
	query.Execute()
	...
	switch(param)
		if("reason")
			...
			var/DBQuery/update_query = GLOB.dbcon.NewQuery("UPDATE erro_ban SET reason = '[value]', edits = CONCAT(edits,'- [eckey] changed ban reason from <cite><b>\\\"[reason]\\\"</b></cite> to <cite><b>\\\"[value]\\\"</b></cite><BR>') WHERE id = [banid]")
		...
		if("duration")
			...
			var/DBQuery/update_query = GLOB.dbcon.NewQuery("UPDATE erro_ban SET duration = [value], edits = CONCAT(edits,'- [eckey] changed ban duration from [duration] to [value]<br>'), expiration_time = DATE_ADD(bantime, INTERVAL [value] MINUTE) WHERE id = [banid]")
...
/datum/admins/proc/DB_ban_panel(var/playerckey = null, var/adminckey = null, var/playerip = null, var/playercid = null, var/dbbantype = null, var/match = null)
	...
	if(adminckey || playerckey || playerip || playercid || dbbantype)
		...
		if(adminckey || playerckey || playerip || playercid || dbbantype)
			...
			var/DBQuery/select_query = GLOB.dbcon.NewQuery("SELECT id, bantime, bantype, reason, job, duration, expiration_time, ckey, a_ckey, unbanned, unbanned_ckey, unbanned_datetime, edits, ip, computerid FROM erro_ban WHERE 1 [playersearch] [adminsearch] [ipsearch] [cidsearch] [bantypesearch] ORDER BY bantime DESC LIMIT 100")
