...
/datum/admin_secret_item/admin_secret/launch_shuttle/can_execute(var/mob/user)
	if(!GLOB.shuttle_controller) return 0
	return ..()
...
/datum/admin_secret_item/admin_secret/launch_shuttle/execute(var/mob/user)
	...
	for (var/shuttle_tag in GLOB.shuttle_controller.shuttles)
		if (istype(GLOB.shuttle_controller.shuttles[shuttle_tag], /datum/shuttle/ferry))
	...
	var/datum/shuttle/ferry/S = GLOB.shuttle_controller.shuttles[shuttle_tag]
	if (S.can_launch())
		S.launch(user)
