/datum/unit_test/uplink_setup_test
	name = "UPLINK: All GLOB.uplink items shall be valid."

/datum/unit_test/uplink_setup_test/start_test()
	var/success = TRUE

	for(var/item in GLOB.uplink.items)
	...
	for(var/item in uplink_selection.items)
		var/datum/uplink_random_item/uri = item // Basically ensuring random GLOB.uplink items is a subset of the full range of items
		success = is_valid_uplink_item(GLOB.uplink.items_assoc[uri.uplink_item], "Random GLOB.uplink items", uri.uplink_item) && success

	if(success)
		pass("All GLOB.uplink items were valid.")
	else
		fail("One or more GLOB.uplink items were invalid.")
