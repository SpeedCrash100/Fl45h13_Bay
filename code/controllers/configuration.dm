...
//	var/enable_authentication = 0		// goon authentication
	...
	var/fps = 20
	var/resume_after_initializations = 0 //prevents the world from sleeping while no players are connected after initializations
	var/tick_limit_mc_init = TICK_LIMIT_MC_INIT_DEFAULT	//SSinitialization throttling
	var/base_mc_tick_rate = 1
	var/high_pop_mc_tick_rate = 1.1 //High population MC tick rate
	var/high_pop_mc_mode_amount = 60 //Engage high pop mode if player count raises above this (Player in this context means any connected user. Lobby, ghost or in-game all count)
	var/disable_high_pop_mc_mode_amount = 65 //Disengage high pop mode if player count drops below this
