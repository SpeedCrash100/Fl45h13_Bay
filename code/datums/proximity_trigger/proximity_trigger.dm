...
GLOBAL_VAR_CONST(PROXIMITY_EXCLUDE_HOLDER_TURF, 1) //  When acquiring GLOB.turfs to monitor, excludes the turf the holder itself is currently in.
...
/datum/proximity_trigger/New(var/holder, var/on_turf_entered, var/on_turfs_changed, var/range = 2, var/proximity_flags = 0, var/proc_owner)
	...
	turf_selection = GLOB.decls_repository.get_decl(turf_selection)
...
/datum/proximity_trigger/proc/register_turfs()
	...
	GLOB.dir_set_event.register(holder, src, /datum/proximity_trigger/proc/register_turfs) // Changing direction might alter the relevant GLOB.turfs
