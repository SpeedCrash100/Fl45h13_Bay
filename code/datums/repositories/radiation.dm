...
/datum/radiation_source/proc/update_rad_power(var/new_power = null)
	if(new_power == null || new_power == rad_power)
		...
		rad_power = new_power
		if(!flat)
			range = min(round(sqrt(rad_power / GLOB.config.radiation_lower_limit)), 31)  // R = rad_power / dist**2 - Solve for dist
...
/repository/radiation/proc/get_rads_at_turf(var/turf/T)
	...
	for(var/value in sources)
		...
		while(origin != T)
			...
			working = max((working - (origin.cached_rad_resistance * GLOB.config.radiation_resistance_multiplier)), 0)
			if(working <= .)
