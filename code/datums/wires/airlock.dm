...
GLOBAL_VAR_CONST(AIRLOCK_WIRE_IDSCAN, 1)
GLOBAL_VAR_CONST(AIRLOCK_WIRE_MAIN_POWER1, 2)
GLOBAL_VAR_CONST(AIRLOCK_WIRE_MAIN_POWER2, 4)
GLOBAL_VAR_CONST(AIRLOCK_WIRE_DOOR_BOLTS, 8)
GLOBAL_VAR_CONST(AIRLOCK_WIRE_BACKUP_POWER1, 16)
GLOBAL_VAR_CONST(AIRLOCK_WIRE_BACKUP_POWER2, 32)
GLOBAL_VAR_CONST(AIRLOCK_WIRE_OPEN_DOOR, 64)
GLOBAL_VAR_CONST(AIRLOCK_WIRE_AI_CONTROL, 128)
GLOBAL_VAR_CONST(AIRLOCK_WIRE_ELECTRIFY, 256)
GLOBAL_VAR_CONST(AIRLOCK_WIRE_SAFETY, 512)
GLOBAL_VAR_CONST(AIRLOCK_WIRE_SPEED, 1024)
GLOBAL_VAR_CONST(AIRLOCK_WIRE_LIGHT, 2048)
...
/datum/wires/airlock/UpdateCut(var/index, var/mended)
	...
	switch(index)
		...
		if(GLOB.AIRLOCK_WIRE_BACKUP_POWER1, GLOB.AIRLOCK_WIRE_BACKUP_POWER2)
		...
		if(GLOB.AIRLOCK_WIRE_DOOR_BOLTS)
		...
		if(GLOB.AIRLOCK_WIRE_AI_CONTROL)
		...
		if(GLOB.AIRLOCK_WIRE_ELECTRIFY)
			if(!mended)
		...
		if (GLOB.AIRLOCK_WIRE_SAFETY)
			A.safe = mended

		if(GLOB.AIRLOCK_WIRE_SPEED)
			A.autoclose = mended
			if(mended)
				if(!A.density)
					A.close()

		if(GLOB.AIRLOCK_WIRE_LIGHT)
			A.lights = mended
			A.update_icon()
...
/datum/wires/airlock/UpdatePulsed(var/index)
	...
	switch(index)
		...
		if(GLOB.AIRLOCK_WIRE_ELECTRIFY)
			...
			A.electrify(30)
		if(GLOB.AIRLOCK_WIRE_OPEN_DOOR)
