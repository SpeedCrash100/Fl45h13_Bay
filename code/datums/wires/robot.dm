...
/datum/wires/robot/UpdateCut(var/index, var/mended)
	...
	var/mob/living/silicon/robot/R = holder
	switch(index)
		if(GLOB.BORG_WIRE_LAWCHECK) //Cut the law wire, and the borg will no longer receive law updates from its AI
			if(!mended)
				if (R.lawupdate == 1)
		...
		if (GLOB.BORG_WIRE_AI_CONTROL) //Cut the AI wire to reset AI control
			if(!mended)
				R.disconnect_from_ai()

		if (GLOB.BORG_WIRE_CAMERA)
			if(!isnull(R.camera) && !R.scrambledcodes)
				R.camera.status = mended

		if(GLOB.BORG_WIRE_LAWCHECK)	//Forces a law update if the borg is set to receive them. Since an update would happen when the borg checks its laws anyway, not much use, but eh
			if (R.lawupdate)
				R.lawsync()
...
/datum/wires/robot/UpdatePulsed(var/index)
	var/mob/living/silicon/robot/R = holder
	switch(index)
		if (GLOB.BORG_WIRE_AI_CONTROL) //pulse the AI wire to make the borg reselect an AI
			if(!R.emagged)
...
/datum/wires/robot/proc/IsCameraCut()
	return wires_status & GLOB.BORG_WIRE_CAMERA

/datum/wires/robot/proc/LockedCut()
	return wires_status & GLOB.BORG_WIRE_LOCKED_DOWN
