...
/datum/extension/interactive/multitool/items/cable/get_interact_window(var/obj/item/device/multitool/M, var/mob/user)
	...
	. += "<table>"
	for(var/cable_color in GLOB.possible_cable_coil_colours)
		. += "<tr>"
		. += "<td>[cable_color]</td>"
		if(cable_coil.color == GLOB.possible_cable_coil_colours[cable_color])
			. += "<td>Selected</td>"
		else
...
/datum/extension/interactive/multitool/items/cable/on_topic(href, href_list, user)
	var/obj/item/stack/cable_coil/cable_coil = holder
	if(href_list["select_color"] && href_list["select_color"] in GLOB.possible_cable_coil_colours)
