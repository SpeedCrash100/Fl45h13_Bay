...
/decl/communication_channel/proc/can_communicate(var/datum/communicator, var/message)
	...
	if(config_setting && !GLOB.config.vars[config_setting] && !check_rights(R_INVESTIGATE,0,communicator))
...
/proc/communicate(var/channel_type, var/communicator, var/message)
	var/list/channels = GLOB.decls_repository.decls_of_subtype(/decl/communication_channel)
