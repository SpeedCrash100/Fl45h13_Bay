/obj/machinery/computer/shuttle
	...
	attackby(var/obj/item/weapon/card/W as obj, var/mob/user as mob)
		...
		if ((!( istype(W, /obj/item/weapon/card) ) || !( GLOB.ticker ) || GLOB.evacuation_controller.has_evacuated() || !( user )))
			return
		...
		if (istype(W, /obj/item/weapon/card/id)||istype(W, /obj/item/device/pda))
			...
			if(!(GLOB.access_heads in W:access)) //doesn't have this access
				to_chat(user, "The access level of [W:registered_name]\'s card is not high enough. ")
				return 0

			var/choice = alert(user, text("Would you like to (un)authorize a shortened launch time? [] authorization\s are still needed. Use abort to cancel all authorizations.", src.auth_need - src.authorized.len), "Shuttle Launch", "Authorize", "Repeal", "Abort")
			if(GLOB.evacuation_controller.is_prepared() && user.get_active_hand() != W)
				return 0
			switch(choice)
				if("Authorize")
