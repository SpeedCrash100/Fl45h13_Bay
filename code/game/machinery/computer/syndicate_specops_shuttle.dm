...
*/
	...
	for(var/turf/T in get_area_turfs(end_location) )
		...
		to_chat(M, "<span class='warning'>You have arrived at [GLOB.using_map.station_name]. Commence operation!</span>")
...
//		return
	...
	if (temp)
		...
		\nLocation: [GLOB.syndicate_elite_shuttle_moving_to_station || GLOB.syndicate_elite_shuttle_moving_to_mothership ? "Departing for [GLOB.using_map.station_name] in ([GLOB.syndicate_elite_shuttle_timeleft] seconds.)":GLOB.syndicate_elite_shuttle_at_station ? "Station":"Dock"]<BR>
		[GLOB.syndicate_elite_shuttle_moving_to_station || GLOB.syndicate_elite_shuttle_moving_to_mothership ? "\n*The Syndicate Elite shuttle is already leaving.*<BR>\n<BR>":GLOB.syndicate_elite_shuttle_at_station ? "\n<A href='?src=\ref[src];sendtodock=1'>Shuttle Offline</A><BR>\n<BR>":"\n<A href='?src=\ref[src];sendtostation=1'>Depart to [GLOB.using_map.station_name]</A><BR>\n<BR>"]
...
/obj/machinery/computer/syndicate_elite_shuttle/Topic(href, href_list)
	...
	if (href_list["sendtodock"])
		...
		to_chat(usr, "<span class='notice'>The Syndicate Elite shuttle will arrive on [GLOB.using_map.station_name] in [(SYNDICATE_ELITE_MOVETIME/10)] seconds.</span>")
