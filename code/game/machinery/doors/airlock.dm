...
/obj/machinery/door/airlock/uranium/process()
	if(world.time > last_event+20)
		if(prob(50))
			GLOB.radiation_repository.radiate(src, rad_power)
...
*		two wires for power. Sending a pulse through either one causes a breaker to trip, disabling the door for 10 seconds if backup power is connected, or 1 minute if not (or until backup power comes back on, whichever is shorter). Cutting either one disables the main door power, but unless backup power is also cut, the backup power re-GLOB.powers the door in 10 seconds. While unpowered, the door may be open, but bolts-raising will not work. Cutting these wires may electrocute the user.
...
/obj/machinery/door/airlock/requiresID()
	return !(src.isWireCut(GLOB.AIRLOCK_WIRE_IDSCAN) || aiDisabledIdScanner)
...
/obj/machinery/door/airlock/proc/mainPowerCablesCut()
	return src.isWireCut(GLOB.AIRLOCK_WIRE_MAIN_POWER1) || src.isWireCut(GLOB.AIRLOCK_WIRE_MAIN_POWER2)

/obj/machinery/door/airlock/proc/backupPowerCablesCut()
	return src.isWireCut(GLOB.AIRLOCK_WIRE_BACKUP_POWER1) || src.isWireCut(GLOB.AIRLOCK_WIRE_BACKUP_POWER2)
...
/obj/machinery/door/airlock/proc/electrify(var/duration, var/feedback = 0)
	var/message = ""
	if(src.isWireCut(GLOB.AIRLOCK_WIRE_ELECTRIFY) && arePowerSystemsOn())
...
/obj/machinery/door/airlock/proc/set_idscan(var/activate, var/feedback = 0)
	var/message = ""
	if(src.isWireCut(GLOB.AIRLOCK_WIRE_IDSCAN))
...
/obj/machinery/door/airlock/proc/set_safeties(var/activate, var/feedback = 0)
	...
	if (src.isWireCut(GLOB.AIRLOCK_WIRE_SAFETY))
	...
	switch (href_list["command"])
		...
		if("backup_power")
			if(!backup_power_lost_until)
				src.loseBackupPower()
		if("bolts")
			if(src.isWireCut(GLOB.AIRLOCK_WIRE_DOOR_BOLTS))
		...
		if("timing")
			// Door speed control
			if(src.isWireCut(GLOB.AIRLOCK_WIRE_SPEED))
		...
		if("lights")
			// Bolt lights
			if(src.isWireCut(GLOB.AIRLOCK_WIRE_LIGHT))
...
/obj/machinery/door/airlock/can_open(var/forced=0)
	...
	if(!forced)
		if(!arePowerSystemsOn() || isWireCut(GLOB.AIRLOCK_WIRE_OPEN_DOOR))
			return 0
...
/obj/machinery/door/airlock/proc/unlock(var/forced=0)
	...
	if (!forced)
		if(operating || !src.arePowerSystemsOn() || isWireCut(GLOB.AIRLOCK_WIRE_DOOR_BOLTS))
			return
