	...
	New()
		...
		spawn(10)	// must wait for map loading to finish
			if(GLOB.radio_controller)
				GLOB.radio_controller.add_object(src, freq, GLOB.RADIO_MAGNETS)
...
/obj/machinery/magnetic_module/Destroy()
	if(GLOB.radio_controller)
		GLOB.radio_controller.remove_object(src, freq)
	..()
...
/obj/machinery/magnetic_controller
	...
	New()
		...
		spawn(45)	// must wait for map loading to finish
			if(GLOB.radio_controller)
				radio_connection = GLOB.radio_controller.add_object(src, frequency, GLOB.RADIO_MAGNETS)
