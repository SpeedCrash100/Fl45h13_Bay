...
/datum/universal_state/nuclear_explosion/OnEnter()
	if(GLOB.ticker && GLOB.ticker.mode)
		GLOB.ticker.mode.explosion_in_progress = 1
	...
	if(GLOB.ticker && GLOB.ticker.mode)
		GLOB.ticker.mode.station_was_nuked = 1
		GLOB.ticker.mode.explosion_in_progress = 0
		if(!GLOB.ticker.mode.check_finished())//If the mode does not deal with the nuke going off so just reboot because everyone is stuck as is
