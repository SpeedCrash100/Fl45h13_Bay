...
obj/item/device/cable_painter/New()
	..()
	color_selection = pick(GLOB.possible_cable_coil_colours)
...
obj/item/device/cable_painter/attack_self(mob/user)
	var/new_color_selection = input("What color would you like to use?", "Choose a Color", color_selection) as null|anything in GLOB.possible_cable_coil_colours
...
/obj/item/device/cable_painter/afterattack(var/atom/A, var/mob/user, var/proximity)
	...
	if(istype(A, /obj/structure/cable))
		var/picked_color = GLOB.possible_cable_coil_colours[color_selection]
