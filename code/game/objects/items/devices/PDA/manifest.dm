...
/datum/datacore/proc/get_manifest_list()
	...
	for(var/datum/data/record/t in GLOB.data_core.general)
		...
		if(GLOB.using_map.flags & GLOB.MAP_HAS_BRANCH && t.fields["mil_branch"] && t.fields["mil_branch"] != "None")
			var/datum/mil_branch/branch_datum = GLOB.mil_branches.get_branch(t.fields["mil_branch"])
			if(branch_datum)
				mil_branch = list("full" = branch_datum.name, "short" = branch_datum.name_short)

		if(GLOB.using_map.flags & GLOB.MAP_HAS_RANK && t.fields["mil_rank"] && t.fields["mil_rank"] != "None")
			var/datum/mil_rank/mil_rank_datum = GLOB.mil_branches.get_rank(t.fields["mil_branch"], t.fields["mil_rank"])
			if(mil_rank_datum)
