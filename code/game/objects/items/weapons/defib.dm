...
/obj/item/weapon/shockpaddles/standalone/checked_use(var/charge_amt)
	GLOB.radiation_repository.radiate(src, charge_amt/12) //just a little bit of radiation. It's the price you pay for being powered by magic I guess
	return 1

/obj/item/weapon/shockpaddles/standalone/process()
	if(fail_counter > 0)
		GLOB.radiation_repository.radiate(src, fail_counter--)
	else
		GLOB.processing_objects.Remove(src)
