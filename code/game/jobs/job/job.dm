...
/datum/job/proc/setup_account(var/mob/living/carbon/human/H)
	...
	if(!(H.species && (H.species.type in GLOB.economic_species_modifier)))
		return //some bizarre species like shadow, slime, or monkey? You don't get an account.

	var/species_modifier = GLOB.economic_species_modifier[H.species.type]
...
/datum/job/proc/is_branch_allowed(var/branch_name)
	if(!allowed_branches || !GLOB.using_map || !(GLOB.using_map.flags & GLOB.MAP_HAS_BRANCH))
		return 1
	if(branch_name == "None")
		return 0
...
/datum/job/proc/is_rank_allowed(var/branch_name, var/rank_name)
	if(!allowed_ranks || !GLOB.using_map || !(GLOB.using_map.flags & GLOB.MAP_HAS_RANK))
		return 1
