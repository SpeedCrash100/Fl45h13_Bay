...
/datum/job/chaplain
	...
	equip(var/mob/living/carbon/human/H, var/alt_title, var/ask_questions = TRUE)
		...
		spawn(1)
			...
			if(GLOB.ticker)
				GLOB.ticker.Bible_icon_state = B.icon_state
				GLOB.ticker.Bible_item_state = B.item_state
				GLOB.ticker.Bible_name = B.name
				GLOB.ticker.Bible_deity_name = B.deity_name
