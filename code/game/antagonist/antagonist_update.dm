...
/datum/antagonist/proc/update_current_antag_max()
	...
	if(GLOB.ticker && GLOB.ticker.mode)
		if(GLOB.ticker.mode.antag_tags && (id in GLOB.ticker.mode.antag_tags))
			cur_max = hard_cap_round

	if(GLOB.ticker.mode.antag_scaling_coeff)
		...
		cur_max = max(initial_spawn_target,min(round(count/GLOB.ticker.mode.antag_scaling_coeff),cur_max))
