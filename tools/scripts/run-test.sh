...
function run_byond_tests {
    ...
    if [[ "$CI" == "true" ]]; then
        ...
        tools/scripts/install-byond.sh || exit 1
...
#    run_test "check globals unchanged" "md5sum -c - <<< 'ee783dd5614e829cc42ca2bbd17ce781 *code/_helpers/global_access.dm'"
    run_test "build map unit tests" "tools/scripts/dm.sh -DUNIT_TEST -M$MAP_PATH baystation12.dme"
